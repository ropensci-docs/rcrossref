...
- doi:
  ...
  (character) One or more digital object identifiers. If `async=FALSE`
  we do synchronous HTTP requests in an `lapply` call, but if
  `async=TRUE`, we do asynchronous HTTP requests.
...
- key:
  ...
  your Crossref OpenURL email address, either enter, or loads from
  `.Rprofile`. We use a default, so you don't need to pass this.
...
a data.frame, with columns `doi` and `count`. The count column has
numeric values that are the citation count for that DOI, or `NA` if not
found or no count available
...
I would imagine it's best to use this data instead of from the
publishers, and this data you can get programatically :)

## failure behavior

When a DOI does not exist, we may not get a proper HTTP status code to
throw a proper stop status, so we grep on the text itself, and throw a
stop if only one DOI passed and not using async, or warning if more than
one DOI passed or if using async.

## See also

[`cr_search()`](https://docs.ropensci.org/rcrossref/reference/cr_search-defunct.md),
[`cr_r()`](https://docs.ropensci.org/rcrossref/reference/cr_r.md)
