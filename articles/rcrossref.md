...
CrossRef’s DOI Content Negotiation service, where you can citations back
in various formats, including `apa`
...
There are a lot more styles. We include a dataset as a character vector
within the package, accessible via the
...
[`cr_r()`](https://docs.ropensci.org/rcrossref/reference/cr_r.md) uses
the function
...
Publishers can optionally provide links in the metadata they provide to
Crossref for full text of the work, but that data is often missing. Find
out more about it at
<https://support.crossref.org/hc/en-us/articles/215750183-Crossref-Text-and-Data-Mining-Services>

Get some DOIs for articles that provide full text, and that have
`CC-BY 3.0` licenses (i.e., more likely to actually be open)
...
From the output of `cr_works` we can get full text links if we know
where to look:
...
See also fulltext (<https://github.com/ropensci/fulltext>) for getting
scholarly text for text mining.
